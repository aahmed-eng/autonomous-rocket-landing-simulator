function PlotGraphs(A, V, H, FM, T, k)
...
% Inputs: vectors of accelertion, velocity, height, fuel mass, time and the
% number of steps.
...
hold on
plot(T, V, 'b')
plot(T(k-1), V(k-1), 'r*') % plot landing velocity
hold off

% Add text to explain landing velocity point
text(1.02*T(k-1), V(k-1), "Landing Velocity = " + V(k-1) + "ms^{-1}")
...
text(1.02*T(minFMIdx), minFM, "Minimum fuel mass = " + minFM + "kg")
ylabel('fuel mass (kg)')
xlabel('time (s)')
grid on

sgtitle('Build 3: Variable Mass, Constant Thrust, Variable Drag Model')
